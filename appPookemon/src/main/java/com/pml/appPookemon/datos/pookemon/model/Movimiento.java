...
public abstract class Movimiento implements Serializable {
    ...
    public Movimiento(int idMovimiento, String nombre, int potencia, int precision, int cantidadPP, String elemento, Efecto efecto, int probabilidadEfecto) {
    ...
    public void realizar(Pookemon atacante, Pookemon defensor) {
        if (atacante.puedeAtacar()) {
            ...
            double probabilidadEfecto = random.nextDouble() * 100;
            if (probabilidadEfecto < this.probabilidadEfecto) {
                defensor.setEfecto(efecto);
            }
        }
    }
